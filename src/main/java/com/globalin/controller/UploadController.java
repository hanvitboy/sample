...
public class UploadController {
	...
	//원래는 byte[]배열로 처리해야하지만 spring 에서 조금더 쉽게 할수있도록 resource
	...
	//서버에서 get방식으로 이미지파일을 가져올 수 있도록 처리
	//특정한 uri 뒤에 파일 이름을 추가하면 이미지 파일 데이터를 가져와서 <img>태그를 작성
	//우리가 생각해야 할 점 : 썸네일 이미지의 이름 '파일경로' + 's_' + 'uuid' + '파일의 원래이름'
	//그리고 한글 처리까지
	 ...
	 * media type 을 통해 브라우저에게 파일의 종류를 알려줘야 한다.
	 * 브라우저에게 알려주는 방법은 응답의 헤더정보에 포함 시켜야함
	 * */
		//헤더 객체 생성
		try {
	...
	public void uploadForm() {
		//리턴이 void므로 url 이 파일 이름이된다.- > uploadForm.jsp
	}
	...
	public void uploadFormAction(MultipartFile[] uploadFile, Model model) {
		for(MultipartFile file: uploadFile) {
			...
			//log.info(file.getName()); 이것과헷갈리지말것. input tag의 name속성을 반환
	...
	//responseentity<list<attachfile>> 타입으로
	...
	public ResponseEntity<List<AttachFile>> uploadAjaxAction(MultipartFile[] uploadFile) {
		...
		//섬네일을 만들때 해당 파일이 이미지 파일인지를 먼저 검사할 필요가 있다.
		//이미지 파일 검사 메소드
	private boolean checkImageType(File file) {
		//파라미터로 받아온 file이 이미지 파일이면 return true;
		//파라미터로 받아온 file이 이미지가 아니면 false;
