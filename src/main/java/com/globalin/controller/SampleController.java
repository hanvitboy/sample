...
public class SampleController {
	...
	/*
	 * RestController 는 jsp 와 달리 순수한 데이터를 반환해준다.
	 * 다양한 형식의 데이터를 전송할수있는데 주로 많이사용하는 형태는
	 * JSON, 문자열 , XML 이다.
	 * 
	 * */
	
	//문자열을 반환하는 형태
	//controller는 리턴타입이 string 인 메소드:
	...
	//객체를 반환 하겠다 json이나 xml을 이용한다.
	@GetMapping(value="/getSample"
			//,produces = MediaType.APPLICATION_JSON_UTF8_VALUE 이걸쓰면 json 안쓰며 xml 로나옴
			)
	...
	//? 파라미터 = 값 : 쿼리스트링
	// 파라미터값
	//이 요청은 요청 url에 파라미터 값이 포함되어있고 각 파라미터의이름은 cat pid이다. 값이 cat pid는 아님
