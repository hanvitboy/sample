...
<div class="row">
	<div class="col-lg-12">
		...
		<div class="panel-body">
			...
			<!-- 속성선택자 button[data-oper='modify'] -->
			<button data-oper="list" class="btn btn-info">List</button>
			<form id="operForm" action="/board/modify" method="get">
				<!-- 게시물 번호 bno 숨기기 -->
				<input type="hidden" id="bno" name="bno" value="<c:out value='${board.bno }'/>"/>
				<!-- 우리가 보고있던 페이지 정보 숨겨 놓기 -->
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-group">
					<label>Reply</label>
					<input class="form-control" name="reply" placeholder="댓글 내용">
				</div>
				<div class="form-group">
					<label>Replyer</label>
					<input class="form-control" name="replyer" placeholder="작성자" readonly>
				</div>
			</div>
			<!-- 버튼 모음(수정,삭제,등록,닫기....) -->
	...
	//목록을 불러올때는 페이지 정보가 필요한데, 필요한 정보들은 모두 param에 넣을것
		...
		function displayTime(timeValue){
			...
			if(gap <(1000*60*60*24)){
				...
				//현재시간이 9보다크면 앞에아무것도안붙이고 9이하면 09시로 만들어줌
				str = [(hh>9 ? '' : '0')+ hh, ':' , 
						(mi >9 ? '' : '0')+mi, ':',
						(ss>9 ? '' : '0')+ss].join(""); //배열원소들이 이어붙여진다.
			
			} else{
				// 여기는 24시간 이전에 작성한 댓글
				//날짜로 보여주기
				var yy = dateObj.getFullYear();
				var mm = dateObj.getMonth() + 1; //컴퓨터는 달을 0부터센다.
 ...
 */
 //getList함수 {param,callback,error}
...
$(document).ready(function(){
	...
	//ul.html(dddd) :ul태그안의 html 코드를 직접 수정
	...
	function showList(page){
		//겟리스트의 파라미터는 :페이지정보, 게시글번호
		replyService.getList({
			bno : bnoValue,
			page : page || 1
		}, function(replyCnt , list){
			//요청 성공시 callback함수 list를 받어옴
			
			console.log("replyCnt : " + replyCnt);
			console.log("list : " + list);
			//만약 페이지번호가 -1로 전달되면 마지막 페이지로 가도록 한다.
			if(page == -1){
				//댓글 갯수가 7개일때 페이지는 1개
	...
	function showReplyPage(replyCnt){
		...
		//만약 endNum(마지막페이지) 포함 댓글 갯수가 실제 댓글 갯수보다 많으면
		if(endNum * 10 >= replyCnt){
			endNum = Math.ceil(replyCnt/10.0);
		}
		//endNum 가 댓글 갯수보다 적으면 다음 페이지가 있다. next = true
		if(endNum * 10 < replyCnt){
			next = true;
		}
	...
	$("#addReplyBtn").on("click", function(e){
		//#addReplyBtn 은 덧글 추가하기 이므로 수정, 삭제 버튼 필요없음. 보여줄필요없다
		
		//name 이 replyer인 input tag의 값을 우리가 위에서 가져온 replyer로 설정할것
	...
	// 그li 태그들은 현재 스크립트 안에서는 찾을 수가 없다.
	// li 태그가 생성된 이후에 이벤트를 달아야 한다.
	// 지금 여기서 li 태그에 이벤트를 달려고 하면 찾을 수없다.
	// 이벤트 위임
	...
	//수정하려는 사람이 원래 작성자랑 다르면 알림창 듸우고 리턴
	...
	$("#modalRemoveBtn").on("click", function(e){
		...
		//replypageFotter 안의 li a 요소에다가 이벤트 대상 변경해주기
